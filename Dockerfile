...
RUN env GOOS=$TARGETOS GOARCH=$TARGETARCH go build -modcacherw -o extend-async-messaging_$TARGETOS-$TARGETARCH
...
COPY --from=builder /build/extend-async-messaging_$TARGETOS-$TARGETARCH extend-async-messaging
...
# Entrypoint.
CMD [ "/app/extend-async-messaging" ]